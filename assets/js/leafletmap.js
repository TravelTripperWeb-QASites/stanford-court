  ...
  $.fn.leafMap = function (options) {
    ...
    function loadAttractionMarkers() {
      for (var i = 0; i < attractionsArray.length; i++) {
        ...
        marker.on('click', markerClickDefaultAction);
        var attractionObj = {
    ...
    function markerClickDefaultAction(e) {
      var mlat = e.target._latlng.lat;
      var mlng = e.target._latlng.lng;
      map.panTo({
        lat: mlat,
        lng: mlng
      });
      if(options.markerClickAction && options.markerClickAction instanceof Function) {
        options.markerClickAction(e);
      }
    }
